...
<head>
    ...
    <style>
        ...
        /* Custom styles for file upload input */
        .file-upload {
            position: relative;
            overflow: hidden;
...
<body class="bg-gradient-to-br from-gray-900 to-black text-white">
    ...
    <section class="bg-black bg-opacity-25 py-10">
        <div class="max-w-lg mx-auto bg-gray-800 bg-opacity-50 rounded-xl p-8">
            <h2 class="text-3xl font-bold text-center mb-6">Try now</h2>
            <form action="{{ route('trynow.upload') }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="flex justify-center mb-6">
                    <div class="file-upload">
                        <label for="pcap_file" class="bg-blue-900 hover:bg-blue-900 text-white font-bold py-2 px-4 rounded cursor-pointer">Upload your file</label>
                        <input type="file" id="pcap_file" name="file" class="upload" accept=".pcap,.pcapng" required>
                    </div>
                </div>
                <div class="flex justify-center mb-6">
                    <div class="Analyze">
                        <button type="submit" class="bg-blue-900 hover:bg-blue-900 text-white font-bold py-2 px-4 rounded">Analyze</button>
                    </div>
                </div>
            </form>
